...
test('active debuffs become plain persistent status labels',()=>{const out=statuses({silenced:true,guardJammed:true,fogged:true,exposed:true,allin:false});assert.deepEqual(Array.from(out,x=>x.code),['増額できない','守りなし','情報なし','手札が見えている']);assert.ok(out.every(x=>x.copy&&x.kind==='debuff'));});
