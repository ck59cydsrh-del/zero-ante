...
test('32 unique abilities, stronger winner pool, common loser pool, no duplicates',()=>{
 assert.equal(R.catalog.length,32);assert.equal(new Set(R.catalog.map(m=>m.id)).size,32);
